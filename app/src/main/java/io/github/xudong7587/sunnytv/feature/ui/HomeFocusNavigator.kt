        ...
        scope.launch {
            try {
                ...
                if (from != to) {
                    ...
                    if (region != null && viewport.height > 0f) {
                        val delta = ViewportPolicy.reveal(region.top, region.height,
                            viewport.top + topInsetPx, viewport.bottom, region.height + 6f)
                        if (motion.enabled) list.animateScrollBy(delta, motion.fade(360)) else list.scrollBy(delta)
                    } else if (to == HomeFocusPlan.LIBRARIES) revealLibraries()
                    else list.revealItem(destination.lazyIndex, motion, topInsetPx.toFloat())
                }
