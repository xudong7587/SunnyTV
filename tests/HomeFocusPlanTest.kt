...
class HomeFocusPlanTest {
    ...
    @Test fun selectedLibraryTargetsItsOwnSourceScopedLatestRegion() {
        val sections=Plan.sections(libraries,false)
        for (key in libraries) assertEquals(Plan.latest(key), Plan.down(sections,Plan.LIBRARIES,"library:$key"))
        assertNotEquals(Plan.down(sections,Plan.LIBRARIES,"library:a:lib0"), Plan.down(sections,Plan.LIBRARIES,"library:b:lib0"))
    }
    @Test fun nextUpRemainsReachableBeforeSelectedLibrary() {
        val sections=Plan.sections(libraries,true)
        assertEquals(Plan.NEXT_UP,Plan.down(sections,Plan.LIBRARIES,"library:b:lib0"))
        assertEquals(Plan.latest("b:lib0"),Plan.down(sections,Plan.NEXT_UP,"library:b:lib0"))
